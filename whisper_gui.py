...
class TranscriptionApp:
    ...
    def create_transcription_widgets(self):
        ...
        # 控制按钮
        self.start_btn = ttk.Button(self.transcription_frame, text="开始听写", command=self.start_transcription)
    ...
    def create_ai_translation_widgets(self):
        ...
        # 模型参数框架
        params_frame = ttk.LabelFrame(self.ai_translation_frame, text="模型参数(温度越高模型的回答越发散)")
    ...
    def run_transcription(self):
        try:
            ...
            print('语音识别库配置完毕，将开始听写 parameters are set,  about to start transcribe...')
    ...
    def generate_subtitles(self, segments, base_name, file_path):
        try:
            ...
            # 生成适合ai翻译的字幕格式
            # forAI_file_name = f"{base_name}_forAI"+'.txt'            
            # forAI_file_path = os.path.join(os.path.dirname(file_path), forAI_file_name)    
