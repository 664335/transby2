...
class TranscriptionApp:
    ...
    def create_transcription_widgets(self):
        ...
        # 是否启用AI翻译勾选框
        ai_frame = ttk.LabelFrame(self.transcription_frame, text="字幕后处理选项")
        ...
        ttk.Checkbutton(ai_frame, text="AI分时段总结", variable=self.enable_segment_summary, 
    ...
    def create_ai_translation_widgets(self):
        ...
        # 服务商选择框架下拉菜单
        self.provider_button = ttk.Menubutton(provider_frame, text=self.provider_var.get(), width=10)
...
"""
    ...
    # def export_segment_results(self):
    #     """导出一段时间总结结果"""
    ...
    #         initialfile="segment_summary.txt"
    #     )
    ...
    def on_ai_translation_toggle(self):
        ...
        if self.enable_ai_translation.get():
            ...
            if success:
                ...
                if message == "用户取消解密":
                    # 用户取消解密，取消勾选状态
                    self.enable_ai_translation.set(False)
                    self.log("用户取消解密，AI翻译选项未启用")
                else:
                    # 其他错误，取消勾选状态
                    self.enable_ai_translation.set(False)
                    self.log(f"API密钥准备失败: {message}，AI翻译选项未启用")
        else:
            self.log("AI翻译未启用")
    ...
    def on_segment_summary_toggle(self):
        ...
        if self.enable_segment_summary.get():
            ...
            if success:
                ...
                if message == "用户取消解密":
                    # 用户取消解密，取消勾选状态
                    self.enable_segment_summary.set(False)
                    self.log("用户取消解密，AI分时段总结选项未启用")
                else:
                    # 其他错误，取消勾选状态
                    self.enable_segment_summary.set(False)
                    self.log(f"API密钥准备失败: {message}，AI分时段总结选项未启用")
        else:
            self.log("AI分时段总结未启用")
    ...
    def enhance_voice_comprehensive(self, input_file, output_file, model_path):
        """
        综合语音增强：降噪 + 低频切除 + 均衡器
        """
        filter_chain = (
    ...
    def run_transcription(self):
        try:
            ...
            # 调用语音增强函数
            self.enhance_voice_comprehensive(temp_audio_path_undenoise, enhanced_audio_path, "./lq.rnnn") 

            # self.log(f"完成语音增强，输出文件: {enhanced_audio_path}")
            ...
            if self.enable_segment_summary.get():   
                def check_and_start_segment_summary():
                    success, message = self.ensure_api_key_ready()  
                    if success:                            
                # 启动时间段总结线程
                        threading.Thread(target=self.run_segment_summary_analysis, daemon=True).start()
                    else:
                        self.progress_queue.put(("error", f"AI分段总结无法启动: {message}")) 

                self.root.after(0, check_and_start_segment_summary)
...
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n"""
    ...
    def update_transcription_progress(self,progress, current_time=None, total_time=None):
        message = f"转录进度: {progress}%"
        if current_time and total_time:
            message += f" ({current_time:.1f}s / {total_time:.1f}s)"
        self.progress_queue.put(("progress", progress))
    ...
    def check_preset_if_modified(self):
        ...
        if self.current_preset in self.presets:
            ...
            current_model = self.ai_model  # 直接使用变量，而不是ComboBox
